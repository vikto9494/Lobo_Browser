...
public class ComponentSource implements NavigatorWindowListener {
	...
	public ComponentSource(final NavigatorWindow window) {
		...
		taggedBookmarksMenu.setToolTipText("Shows up to " + PREFERRED_MAX_MENU_SIZE + " tags with up to " + PREFERRED_MAX_MENU_SIZE + " recent bookmarks each.");
	...
	public Component[] getStatusBarComponents() {
		return new Component[] {
			this.window.createGap(),
			this.getStatusMessageComponent(),
			this.window.createGap(),
			this.getProgressBar(),
			this.window.createGap()
		};
	...
	public JMenu getFileMenu() {
		...
		openMenu.add(menuItem("Blank Window", 'B', KeyStroke.getKeyStroke(KeyEvent.VK_B, KeyEvent.CTRL_MASK | KeyEvent.SHIFT_MASK), this.actionPool.blankWindowAction));
	...
	public JMenu getPageServicesMenu() {
		...
		menu.add(this.navMenuItem("Similar Pages (Google)", "Performs a Google 'related' search.", "http://www.google.com/search?q=related:", true));
		menu.add(this.navMenuItem("Wayback Machine", "Takes you to historical snapshots of the site via archive.org.", "http://web.archive.org/web/*/", false));
		menu.add(this.navMenuItem("Alexa Site Info", "Gives traffic and other information about the site.", "http://www.alexa.com/data/details/main?url=", true));
		return menu;
	}
	...
	private JMenuItem navMenuItem(String title, String toolTip, String urlPrefix, boolean urlEncode) {
		JMenuItem item = new JMenuItem();
	...
	public void populateRecentBookmarks() {
		...
		Collection<HistoryEntry<BookmarkInfo>> historyEntries = BookmarksHistory.getInstance().getRecentEntries(PREFERRED_MAX_MENU_SIZE);		
	...
	public void populateTaggedBookmarks() {
		...
		bookmarksMenu.removeAll();
		Collection<BookmarkInfo> bookmarkInfoList = BookmarksHistory.getInstance().getRecentItemInfo(PREFERRED_MAX_MENU_SIZE * PREFERRED_MAX_MENU_SIZE);		
	...
	public void populateRecentHosts() {
		...
		Collection<HostEntry> hostEntries = NavigationHistory.getInstance().getRecentHostEntries(PREFERRED_MAX_MENU_SIZE);
