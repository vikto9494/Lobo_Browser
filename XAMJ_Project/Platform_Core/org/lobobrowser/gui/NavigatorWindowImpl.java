...
//	private final Collection<JMenuItem> sharedMenuItems = new LinkedList<JMenuItem>();
	...
	/**
	 * Changes the {@link WindowFactory} that is used to
	...
	/**
	 * Constructs a PlatformWindowContextImpl. It starts out
	 * by showing a progress window. Later 
	 * a new browser window is obtained given the windowId, or created. 
	 */
	...
	/**
	 * @param windowFeatures Window features formatted as in the window.open() method of Javascript.
	 */
	...
	public void statusUpdated(final NavigatorFrame clientletFrame, final String value) {
		final NavigatorWindowEvent event = new NavigatorWindowEvent(NavigatorWindowImpl.this, NavigatorEventType.STATUS_UPDATED, clientletFrame, value, RequestType.NONE);
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				EVENT.fireEvent(event);
			}
	...
	public void setStatus(NavigatorFrame frame, String value) {
		...
		synchronized(this) {
			if(!Objects.equals(this.status, value)) {
				...
				final NavigatorWindowEvent event = new NavigatorWindowEvent(this, NavigatorEventType.STATUS_UPDATED, frame, actualStatus, RequestType.NONE);
				EventQueue.invokeLater(new Runnable() {
					public void run() {
						EVENT.fireEvent(event);
					}
