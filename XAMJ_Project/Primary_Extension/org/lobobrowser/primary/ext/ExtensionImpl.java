...
public class ExtensionImpl implements NavigatorExtension {
	...
	public static void showError(NavigatorFrame frame, ClientletResponse response, Throwable exception) {
	    if(logger.isLoggable(Level.WARNING)) {
	        logger.log(Level.WARNING, "showError(): An error occurred trying to process document " + (response == null ? "[null]" : response.getResponseURL()), exception);
	    }
	...
	private static String getExceptionMeaning(java.net.URL url, Throwable exception) {
	     if(exception instanceof org.lobobrowser.clientlet.JavaVersionException) {
			...
			return "This exception is thrown when the content expects the user's Java Virtual Machine " +
			       ...
			       "expecting version " + jve.getExpectingVersion() + " whereas the version running " +
			       "the browser is " + System.getProperty("java.version") + ".";
		}
			...
			"to be more up to date than it currently is. In this case the content is " +
			"expecting version " + nve.getExpectingVersion() + " whereas the user agent " + 
			"version is " + UserAgentImpl.getInstance().getVersion() + ".";
		}
		else {
			...
			if(cause instanceof java.net.MalformedURLException) {
				...
				return "The host named '" + ((java.net.UnknownHostException) cause).getMessage() + "' could not be found by the Domain Name Service (DNS).";
			}
				...
				return "A " + ClientletException.class.getSimpleName() + " is thrown by extensions or documents typically to indicate an unexpected state has been encountered.";
			}
			else {
				return "Unknown.";
			}
